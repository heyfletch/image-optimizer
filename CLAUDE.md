...
- Toolbar icons: masters in `~/Dropbox/FDLLC/Assets/Icons/Mac Finder Automator Icons`, vendored at `finder-actions/icons/<AppName>.png` and converted to `Contents/Resources/toolbar.icns` at build time. osacompile's own droplet icon ships in `Assets.car` behind `CFBundleIconName` and outranks any `.icns`, so the build deletes both. Add a PNG named after the app to give a new button an icon.
