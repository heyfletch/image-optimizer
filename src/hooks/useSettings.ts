  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (saved) {
          // height/aspectRatio always overridden (width-only resize), everything else persists
          setSettingsState({ ...defaultSettings, ...saved, height: null, maintainAspectRatio: true });
        }
      } catch (err) {
