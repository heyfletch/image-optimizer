...
for f in "$@"; do
  ...
  if [ $? -eq 0 ]; then
    osascript -e "display notification \"$(printf '%s' "$result" | tr '"' "'")\" with title \"To 1200px\""
  else
    osascript -e "display notification \"$(printf '%s' "Failed: $result" | tr '"' "'")\" with title \"To 1200px\" sound name \"Basso\""
  fi
done
